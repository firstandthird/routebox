...
routebox.register = function (server, options, next) {
    ...
    function getSettings(req) {
        ...
        // if plugins.routebox.pluginCache is set, use those options for the cache instead:
        const cache = (routeCfg && routeCfg.pluginCache) ? routeCfg.pluginCache :
          req.route.settings.cache;
        // by default uses status code 200 is set:
        if (!cache.statuses) {
            cache.statuses = [200];
        }
    ...
    server.ext('onPreHandler', (req, reply) => {
        ...
        // if the 'auth' option is true, then don't cache for authenticated users:
        if (settings.auth && req.auth.isAuthenticated) return reply.continue();
        ...
        return cache._cache.get(buildCacheKey(ident, settings), (err, cached) => {
            if (err) {
    ...
    server.ext('onPreResponse', (req, reply) => {
        ...
        // if 'auth' is true and they were an authenticated user, also don't do anything:
        if (settings.auth && req.auth.isAuthenticated) {
            return reply.continue();
        }
